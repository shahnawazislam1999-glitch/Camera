const CACHE_NAME = 'presence-study-tracker-v11-offline-ai-accuracy';
const SHELL = [
  './',
  './index.html',
...
async function cacheAIResources() {
  ...
  for (const url of AI_RESOURCES) {
    ...
    clients.forEach(client => client.postMessage({
      type: 'AI_CACHE_PROGRESS', done, total: AI_RESOURCES.length, success
    }));
  }
  ...
  clients.forEach(client => client.postMessage({
    ...
    success: success === AI_RESOURCES.length,
    cached: success
  }));
}
  ...
  if (isAIResource) {
    event.respondWith((async () => {
      ...
      try {
        ...
        return new Response('Offline AI resource is not cached yet.', {
          status: 503,
          headers: { 'Content-Type': 'text/plain' }
        });
      }
